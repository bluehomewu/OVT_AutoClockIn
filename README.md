...
v5.9 新增 Bot 雙向指令支援（`/ping`、`/help`、`/list`、`/list_testsite`）。
...
服務啟動後會在背景自動監聽 Telegram 訊息（long-polling）。  
對 Bot 傳送以下指令即可互動：

| 指令 | 說明 |
|------|------|
| `/ping` | 檢查機器人是否在線，回報今日打卡狀態及主機到正式站 / 測試站的 ping 延遲 |
| `/list` | 列出今日**正式站**排程時間 + EIP 實際打卡記錄（上下班）|
| `/list_testsite` | 列出今日**測試站** EIP 上下班打卡時間 |
| `/help` | 顯示所有可用指令說明 |

> **注意**：`--now` 手動打卡模式不啟動 Bot 指令監聽，指令僅在排程模式（systemd 服務）下有效。
